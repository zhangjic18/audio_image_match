...
def main(args):
    train_dataloader = DataLoader(dataset=audio_classify_dataset(flag="train"), batch_size=args.batch_size, shuffle=True)
    val_dataloader = DataLoader(dataset=audio_classify_dataset(flag="val"), batch_size=args.batch_size, shuffle=False)
    test_dataloader = DataLoader(dataset=audio_classify_dataset(flag="test"), batch_size=args.batch_size, shuffle=False)

    if args.model == "resnet18":
        model = resnet18(num_classes=10, in_channels=8, initial_stride=(1, 4))
    elif args.model == "resnet34":
        model = resnet34(num_classes=10, in_channels=8, initial_stride=(1, 4))
    ...
    if args.scheduler == "MultiStepLR":
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=list(range(40, args.epoch, 40)), gamma=0.1)
    else:
...
def train(args, model, train_dataloader, val_dataloader, device, criterion, optimizer, scheduler):
    ...
    for epoch in range(args.epoch):
        ...
        if val_accuracy > best_val_accuracy:
            torch.save(model, "../checkpoints/" + args.model + "__to_audio_classify.pth")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="audio classify")
